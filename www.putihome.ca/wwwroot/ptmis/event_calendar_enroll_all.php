...
<head>
		...
		$(function(){
			  ...
			  ctt = new LWH.cTABLE({
											...
											url:			"ajax/event_calendar_enroll_all_select.php",
		...
		function full_ajax() {
			...
			$("form[name='form_register']").attr("action", "<?php echo $CFG["http"] . $CFG["admin_domain"];?>/puti_registration6.php");
		...
		function quick_ajax() {
			...
			$("input[name='enroll_onsite']").val( $("#onsite").is(":checked")?1:0  );
			$("input[name='enroll_trial']").val( $("#trial").is(":checked")?1:0 );
			$("form[name='form_register']").attr("action", "<?php echo $CFG["http"] . $CFG["admin_domain"];?>/puti_qform6.php");
