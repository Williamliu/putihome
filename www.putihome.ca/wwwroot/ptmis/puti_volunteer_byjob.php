...
<head>
		...
		function list_event() {
			...
			$.ajax({
				...
				error: function(xhr, tStatus, errorTh ) {
					$("#wait").loadHide();
					alert("Error (puti_volunteer_byjob_select.php): " + xhr.responseText + "\nStatus: " + tStatus);
				},
				...
				type: "post",
				url: "ajax/puti_volunteer_byjob_select.php"
			});
		}
		...
		function print_event() {
				  if( $("iframe[name='ifm_list_excel']").length > 0 ) {
						...
						$("form[name='frm_list_excel']").attr({"action":"ajax/puti_volunteer_byjob_output.php", "target": "ifm_list_excel" }); 
		...
		function jsonToHTML( obj ) {
				...
				var c41 = ' style="background-color:#BFF1F8; text-align:right;"';
				var c5 = ' style="background-color:#7AC8D8;"';
				
				var html = '<table id="mytab"  class="tabQuery-table" border="1" cellpadding="1" cellspacing="0">';
				html += '<tr>';
				html += '<td colspan="9" align="center"><span style="font-size:12px; font-weight:bold;">义工工时报表按部门统计<br>' + obj.period + '</span></td>';
				
				html += '</tr>';
				html += '<tr>';
				html += '<td colspan="5" class="tabQuery-table-header" ' + h11 + '>部门</td>';
				html += '<td class="tabQuery-table-header" ' + h1 + '></td>';
				...
				if( level >= 2) {
					...
					html += '<td colspan="4" class="tabQuery-table-header" ' + c21 + '>工作内容</td>';
					...
					html += '<td class="tabQuery-table-header" ' + c2 + '>人数</td>';
					html += '</tr>';
				}
				if( level >= 3) {
					html += '<tr>';
					html += '<td class="tabQuery-table-header" colspan="2"></td>';
					html += '<td class="tabQuery-table-header" ' + c3 + '>中文名</td>';
					html += '<td class="tabQuery-table-header" ' + c3 + '>英文名</td>';
					html += '<td class="tabQuery-table-header" ' + c3 + '>法名</td>';
					html += '<td class="tabQuery-table-header" ' + c3 + '></td>';
					html += '<td class="tabQuery-table-header" ' + c3 + '>次数</td>';
					html += '<td class="tabQuery-table-header" ' + c3 + '>总工时</td>';
					html += '<td class="tabQuery-table-header"></td>';
					html += '</tr>';
				}
				...
				if( level >= 4) {
				  html += '<tr>';
				  html += '<td colspan="4" class="tabQuery-table-header"></td>';
				  html += '<td colspan="2" class="tabQuery-table-header" ' + c41 + '>服务内容</td>';
				  html += '<td class="tabQuery-table-header" ' + c4 + '>服务日期</td>';
				  html += '<td class="tabQuery-table-header" ' + c4 + '>工时</td>';
				  html += '<td class="tabQuery-table-header"></td>';
				  html += '</tr>';
				}
				...
				for(var idx in obj.list) {
					...
					//level 22222 
					if( level >= 2) {
						for(var idx2 in dObj.jobs) {
							  var jObj = dObj.jobs[idx2];
							  html += '<tr>';
							  html += '<td></td>';
							  ...
							  html += '<td align="right"' + c2 + '>';
							  html += jObj.work_count;
							  html += '</td>';
	
							  html += '<td align="right"' + c2 + '>';
							  html += jObj.total_hour;
							  html += '</td>';
	
							  html += '<td align="right"' + c2 + '>';
							  html += jObj.total_head;
							  html += '</td>';
							  ...
							  // level 3333
							  if( level >= 3) {
									for(var idx3 in jObj.volunteer) {
										  var vObj = jObj.volunteer[idx3];
										  html += '<tr>';
										  ...
										  // level 4444
										  if(level >= 4) {
											  for(var idx4 in vObj.detail) {
													var tObj = vObj.detail[idx4];
													html += '<tr>';
				...
				html += '<tr>';
				html += '<td colspan="5" align="left"' + c5 + '>';
				html += '</td>';

				html += '<td align="right"' + c5 + '><b>';
				html += '汇总:';
				html += '</b></td>';
                          ...
                          <table cellpadding="2" cellspacing="2">
                              ...
                              <tr>
                                  ...
                                  <td>
                                      ...
                                      <input type="radio" name="report_level" id="level_job"	 value="2" /><label for="level_job"><?php echo $words["summary by dep, job"]?></label><br /> 
                                      <input type="radio" name="report_level" id="level_volunteer"	 value="3" /><label for="level_volunteer"><?php echo $words["summary by dep, job, vol"]?></label><br /> 
                                      <input type="radio" name="report_level" id="level_detail" value="4" /><label for="level_detail"><?php echo $words["summary by dep, job, vol, det"]?></label>
                                  </td>
                              </tr>
