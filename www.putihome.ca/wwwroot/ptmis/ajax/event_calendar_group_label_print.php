...
try {
	...
	*/
	...
	while( $row = $db->fetch($result)) {
		$info["etitle"] 		= $etitle;
		$info["edate"] 			= $edate;
		$info["name"] 			= cTYPE::gstr((strlen($row["first_name"])>9?substr($row["first_name"],0,9 ) . "." : $row["first_name"])   . " " . substr($row["last_name"],0,1) . ".");
		$info["dharma_name"] 	= cTYPE::gstr($row["dharma_name"]);	
