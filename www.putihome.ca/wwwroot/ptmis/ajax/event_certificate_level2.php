...
try {
	...
	while( $row = $db->fetch($result)) {
		...
		if( max(mb_strlen($fname,"utf-8"), mb_strlen($lname,"utf-8")) > 13 ) {
			$maginleft = ( -15 * (max(strlen($fname), strlen($lname))-13) ) . "px";
		} else {
			$maginleft = "0px";
		}
		...
		if( cTYPE::iifcn($fname) && $dname !="" )
			$html .= '<div align="center" style="position:relative;padding-top:10px; height:35px; margin-left:' . $maginleft . '; border:0px solid black;"><span style="font-size:34px;font-family:隶书;">' . $line_two_name . '</span></div>';
		else 
			$html .= '<div align="center" style="position:relative;padding-top:10px; height:35px; margin-left:' . $maginleft . '; border:0px solid black;"><span style="font-size:24px;font-weight:bold;font-family:Arial Bold,Arial;">' . $line_two_name . '</span></div>';
