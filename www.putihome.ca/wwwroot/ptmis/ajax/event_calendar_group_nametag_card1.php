...
try {
	...
	if($sch_lang != "") {
		$criteria .= ($criteria==""?"":" AND ") . "a.language = '" . $sch_lang . "'";
	}
...
function item($i, $j, $info) {
	...
	global $words;
	global $admin_user;
	$html_ret .= '<div style="position:relative; float:left; border:1px dotted #9E0E94; display:inline-block; margin:1px; text-align:left; width:330px; height:200px; overflow:hidden;">';
	...
	$html_ret .= '<span style="font-size:40px;color:#224279;overflow:hidden;white-space:nowrap;">' . $info["name"] . '</span>';
	$html_ret .= '</td>';
	$html_ret .= '</tr>';
	...
	if( $info["dharma_name"] != "" && $info["dharma_name"]!=$info["name"]) {
		if( $admin_user["lang"] != "en" ) {
			$html_ret .= '<tr>';
			$html_ret .= '<td valign="middle" align="center" style="padding-left:-20px;">';
			$html_ret .= '<span style="font-size:40px;color:#224279;overflow:hidden;white-space:nowrap;">' . $info["dharma_name"] . ' ' . $title . '</span>';
			$html_ret .= '</td>';
			$html_ret .= '</tr>';
		} else {
			if( $title != "" ) {
				$html_ret .= '<tr>';
				$html_ret .= '<td valign="middle" align="center" style="padding-left:-20px;">';
				$html_ret .= '<span style="font-size:40px;color:#224279;overflow:hidden;white-space:nowrap;">' . $title . '</span>';
				$html_ret .= '</td>';
				$html_ret .= '</tr>';
			} else {
				$html_ret .= '<tr>';
				$html_ret .= '<td valign="middle" align="center" style="padding-left:-20px;">';
				$html_ret .= '<span style="font-size:40px;color:#224279;overflow:hidden;white-space:nowrap;">' . $info["dharma_name"] . '</span>';
				$html_ret .= '</td>';
				$html_ret .= '</tr>';
			}
		}
...
function item_photo($i, $j, $info) {
	...
	$html_ret .= '<div style="display:block;width:100%;height:150px;">';
	$html_ret .= '<span style="display:inline-block;width:0px;height:100%;vertical-align:middle;"></span>';
	$html_ret .= $photo_html;
	$html_ret .= '</div>';
	$html_ret .= '</td>';
	$html_ret .= '<td valign="middle" align="center" style="padding-left:-20px;">';
	$html_ret .= '<span style="font-size:36px;color:#224279;overflow:hidden;white-space:nowrap;">' . $info["name"] . '</span>';
	$html_ret .= '</td>';
	$html_ret .= '</tr>';
