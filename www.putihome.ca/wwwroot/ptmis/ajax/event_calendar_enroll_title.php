...
try {
	...
	while( $row_bb = $db->fetch($result_bb)) {
		...
		if( $db->row_nums($result) > 0 )  {
			$query = "UPDATE event_calendar_enroll SET group_no = '0', trial = '0', trial_date = '" . time() . "', onsite = '0', deleted = 0 WHERE event_id = '" . $con["event_id"] . "' AND member_id = '" . $row_bb["id"] . "'";
			$result = $db->query( $query );
			
			$query0 = "SELECT first_name, last_name, dharma_name, alias, phone, cell FROM puti_members WHERE deleted <> 1 AND id = '" . $row_bb["id"] . "'";
			...
			$fields = array();
			$fields["event_id"] 		= $con["event_id"];
			$fields["member_id"] 		= $row_bb["id"];
