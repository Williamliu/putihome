...
try {
	...
	if( $_REQUEST["aflag"]=="2" ) {
		  $html = '<center><div style="position:relative; display:block; width:570px; border:0px solid black; margin:0px; padding:0px;">';
		  $info["etitle"] 		= $etitle;
		  $info["edate"] 		= $edate;
		  $info["name"] 		= "";
		  $info["dharma_name"] 	= "";	
	...
	*/
	...
	while( $row = $db->fetch($result)) {
		if( $first ) {
			$first = false;
			if($cnt==0)  $html .= '<center><div style="position:relative; display:block; width:570px; border:0px solid black; margin:0px; padding:0px;">';
		} else {
			if($cnt==0)  $html .= '<center><div style="position:relative; display:block; width:570px; border:0px solid black; margin:0px; padding:0px; page-break-before:always;">';
		}
...
function item($i, $j, $info) {
	...
	$html_ret .= '<div style="position:relative; float:left; border:1px dotted #9E0E94; display:inline-block; margin:1px; text-align:left; width:260px; height:165px; overflow:hidden;">';
	//$html_ret .= '<img src="/theme/blue/image/background/scard.jpg" style="position:absolute; top:0px; left:0px;" width="367" height="320" />';
	if("N"=="Y") {
		$photo_html = '<img src="' . $CFG["http"] . $CFG["admin_domain"] . '/ajax/lwhUpload_image.php?ts=' . time() . '&size=small&img_id=' .  $info["member_id"] . '" height="130" style="border:0px solid #cccccc; vertical-align:middle;" />';
	} else {
	...
	$html_ret .= '<div style="position:absolute; top:10px; width:100%; height:140px; border:0px solid red;">';
	$html_ret .= '<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">';

	$html_ret .= '<tr>';
	$html_ret .= '<td rowspan="2" valign="middle" width="100" align="center">';
	$html_ret .= '<div style="display:block;width:100%;height:140px;"><span style="display:inline-block;width:0px;height:100%;vertical-align:middle;"></span>';
	$html_ret .= $photo_html;
	$html_ret .= '</div>';
	$html_ret .= '</td>';
	...
	if($_REQUEST["title"]!="") {
		...
		$st_title = $st_temp[0];
		$st_size = $st_temp[1]?$st_temp[1]:32;
		$st_space = $st_temp[1]?0:5;
	}
	
	$html_ret .= '<td valign="top" align="center">';
		$html_ret .= '<span style="font-size:' . $st_size . 'px;font-weight:bold;color:#A01E04;overflow:hidden;white-space:nowrap;font-family:隶书;margin-left:-30px;letter-spacing:' . $st_space. 'px;">' . cTYPE::gstr($st_title) . '</span>';
	$html_ret .= '</td>';
	$html_ret .= '</tr>';
	...
	if( cTYPE::iifcn($name) ) {
		if( mb_strlen($name,"utf-8") >= 4 ) $name = mb_substr($name, 0, 2, 'utf-8') . '<br>' . mb_substr($name, 2, mb_strlen($name,"utf-8") , 'utf-8');
		$html_ret .= '<span style="font-size:48px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;font-family:隶书;margin-left:-20px;">' . $name . '</span>';
	} else {
		$html_ret .= '<span style="font-size:32px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;font-family:隶书;margin-left:-20px;">' . $name . '</span>';
	}
