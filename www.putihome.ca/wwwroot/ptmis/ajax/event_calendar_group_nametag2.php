...
function item($i, $j, $info) {
	...
	$html_ret .= '<div style="position:absolute; top:75px; padding:0px; width:100%; height:160px; border:0px solid red; text-align:left; font-family:Arial; overflow:hidden;">';
	...
	if($info["dharma_name"]!= "") {
		...
		if( cTYPE::iifcn($info["name"]) ) {
			...
			if(strlen($info["name"])>13) $fontsize = "32px"; else $fontsize = "44px"; 
		...
		if($info["dharma_name"]!= "") {
			...
			if( cTYPE::iifcn($info["name"]) ) {
				$html_ret .= '<span style="font-size:50px;font-weight:bold;font-family:隶书;">' . $info["name"] . '&nbsp;</span>';
				 ...
				 if(strlen($info["name"])>16) $fontsize = "32px"; else $fontsize = "44px"; 
				//$fontsize = "44px"; 
				$html_ret .= '<span style="font-size:' . $fontsize . ';font-weight:bold;font-family:隶书;">' . $info["name"] . '&nbsp;</span>';
			}
		} 
