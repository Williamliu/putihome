...
try {
	...
	*/
	...
	while( $row = $db->fetch($result)) {
		if( $first ) {
			$first = false;
			if($cnt==0)  $html .= '<center><div style="position:relative; display:block; width:770px; border:0px solid black; margin:0px; padding:0px;">';
		} else {
			if($cnt==0)  $html .= '<center><div style="position:relative; display:block; width:770px; border:0px solid black; margin:0px; padding:0px; page-break-before:always;">';
		}
		...
		if($_REQUEST["lfname"]=="1") 
			$info["name"]			= trim(cTYPE::gstr(cTYPE::lfname1($names)));
		else 
			$info["name"]			= trim(cTYPE::gstr(cTYPE::cname($names)));
		...
		$i = $cnt % 2;
		$j = floor($cnt / 2);
	    if( $photo_req ) $html .= item_photo($i, $j, $info); else $html .= item_photo($i, $j, $info);
		$cnt++;
		if($cnt >= 6) {
			$html .= '</div></center><br>';
			$cnt=0;
...
function item_photo($i, $j, $info) {
	global $CFG;
	global $words;
	$html_ret .= '<div style="position:relative; float:left; border:0px solid red; display:inline-block; margin:1px; text-align:left; width:381px; height:261px; overflow:hidden;">';
	$html_ret .= '<img src="/theme/blue/image/background/scard10_7.png" style="position:absolute; top:0px; left:0px;" width="380" height="260" />';
	
	$html_ret .= '<div style="position:absolute; top:50px; width:100%; text-align:center; font-family:Arial; font-size:18px; color:#A01E04; overflow:hidden; white-space:nowrap;">' . $info["etitle"] . '</div>';
    ...
    if($info["leader"]=="1" || $info["volunteer"]=="1") {
		...
		$html_ret .= '<div style="position:absolute; top:75px; padding-left:10px; width:100%; text-align:left; font-family:Arial; overflow:hidden;">';
		$html_ret .= '<table width="100%" border="0" cellpadding="0" cellspacing="0">';

		$html_ret .= '<tr>';
		$html_ret .= '<td rowspan="4" valign="middle" width="120"><div style="display:block;width:100%;height:160px;"><span style="display:inline-block;width:0px;height:100%;vertical-align:middle;"></span>';
		$html_ret .= $photo_html;
		$html_ret .= '</div></td>';
		...
		$st_size = 48;
		$st_space = 5;
		$st_title = $words["staff nametag"];
		if($_REQUEST["title"]!="") {
			$st_temp = explode("@", $_REQUEST["title"]);
			$st_title = $st_temp[0];
			$st_size = $st_temp[1]?$st_temp[1]:48;
			$st_space = $st_temp[1]?0:5;
		}

		$html_ret .= '<span style="color:#A01E04; font-family:隶书; font-size:'. $st_size .'px; letter-spacing:' . $st_space. 'px;">' . cTYPE::gstr($st_title) . '&nbsp;</span>';
		...
		$html_ret .= '<td valign="middle" align="center" style="padding-left:-5px;">';
		if($info["dharma_name"]!= "") {
			$html_ret .= '<span style="font-size:44px;font-weight:bold;font-family:隶书;">' . $info["dharma_name"] . '&nbsp;</span>';
			$html_ret .= $info["dharma_pinyin"]!=""?'<br><span style="font-size:24px;">' . $info["dharma_pinyin"] . '</span>':"";
		} else {
			if( cTYPE::iifcn($info["name"]) ) {
				$html_ret .= '<span style="font-size:50px;font-weight:bold;font-family:隶书;">' . $info["name"] . '&nbsp;</span>';
			} else {
				if(strlen($info["name"])>10) $fontsize = "32px"; else $fontsize = "44px"; 
				$html_ret .= '<span style="font-size:' . $fontsize . ';font-weight:bold;font-family:隶书;">' . $info["name"] . '&nbsp;</span>';
			}
		} 
		$html_ret .= '</td>';
		$html_ret .= '</tr>';
		...
		// title
		$html_ret .= '<tr>';
		$html_ret .= '<td valign="top" align="center" style="padding-left:-5px;">';
		...
		if($info["group_no"]>0) {
			...
			$html_ret .= '<span style="font-size:22px;font-weight:bold;color:#A01E04;vertical-align:middle;">' . cTYPE::gstr($words["group"]) . ": " . $info["group_no"] . '</span>';
			// end of group
		...
		$html_ret .= '</table>';
		$html_ret .= '</div>';
		$html_ret .= '<div style="position:absolute; top:236px; width:100%; text-align:center; font-family:Arial; font-size:16px; color:#A01E04;">' . $info["edate"] . '</div>';
	} else {
		// not leader and volunteer case
		$html_ret .= '<div style="position:absolute; top:75px; padding-left:10px; width:100%; text-align:left; font-family:Arial; overflow:hidden;">';
		$html_ret .= '<table width="100%" border="0" cellpadding="0" cellspacing="0">';

		$html_ret .= '<tr>';
		$html_ret .= '<td rowspan="3" valign="middle" width="120"><div style="display:block;width:100%;height:160px;"><span style="display:inline-block;width:0px;height:100%;vertical-align:middle;"></span>';
		$html_ret .= $photo_html;
		$html_ret .= '</div></td>';
		$html_ret .= '<td valign="middle" align="center" style="padding-left:-5px;">';
