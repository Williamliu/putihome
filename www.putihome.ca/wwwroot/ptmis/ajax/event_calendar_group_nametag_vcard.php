...
try {
	...
	$query = "SELECT c.title, c.start_date, c.end_date,
					 d.site_name_cn, d.site_name_en, d.phone_cn, d.phone_en 
					  FROM event_calendar c 
					  INNER JOIN puti_sites d ON (c.site = d.id)
	...
	$site_cn 	= $row["site_name_cn"];
	$site_en 	= $row["site__name_en"];
	$phone_cn 	= $row["phone_cn"];
	$phone_en 	= $row["phone_en"];
	...
	if( $_REQUEST["aflag"]=="2" ) {
		  ...
		  $info["site_name"]    = $admin_user["lang"]!="en"? $site_cn:$site_en;
		  $info["phone"]   		= $admin_user["lang"]!="en"? $phone_cn:$phone_en;
	...
	*/
	...
	$query_class = "SELECT a.start_date, a.end_date, a.title as event_title, b.title as class_title, b.cert_prefix, b.photo,
						   c.site_name_en,  c.site_name_cn, c.phone_cn, c.phone_en  
	...
	$site_cn 	= $row_class["site_name_cn"];
	$site_en 	= $row_class["site_name_en"];
	$phone_cn 	= $row_class["phone_cn"];
	$phone_en 	= $row_class["phone_en"];
	...
	while( $row = $db->fetch($result)) {
		if( $first ) {
			$first = false;
			if($cnt==0)  $html .= '<center><div style="position:relative; display:block; width:740px; border:0px solid black; margin:0px; padding:0px;">';
		} else {
			if($cnt==0)  $html .= '<center><div style="position:relative; display:block; width:740px; border:0px solid black; margin:0px; padding:0px; page-break-before:always;">';
		}
...
function item($i, $j, $info) {
	...
	global $words;
	global $admin_user;
	$html_ret .= '<div style="position:relative; float:left; border:1px dotted #9E0E94; display:inline-block; margin:0px; text-align:left; width:220px; height:305px; overflow:hidden;">';
	$html_ret .= '<img src="/theme/blue/image/background/vnamecard.jpg" style="position:absolute; top:0px; left:0px;" width="220" />';
	$html_ret .= '<div style="position:absolute;top:70px;width:100%;text-align:center;font-family:Arial;font-size:14px;font-weight:bold;color:#A01E04;overflow:hidden;white-space:nowrap;">' . $info["etitle"] . '</div>';
	$html_ret .= '<div style="position:absolute;top:90px; width:100%; text-align:center; font-family:Arial; font-size:12px;font-weight:bold; color:#A01E04;">' . $info["edate"] . '</div>';

	$html_ret .= '<div style="position:absolute; top:110px; width:100%; height:140px; border:0px solid red;">';
	...
	if( cTYPE::iifcn($info["name"]) )
		$html_ret .= '<span style="font-size:42px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;font-family:隶书;">' .$info["name"] . '</span>';
	else
		...
		if( $admin_user["lang"] != "en" ) {
			...
			if( cTYPE::iifcn($info["dharma_name"]) ) {
				if( $info["dharma_pinyin"]== "") {
					...
					$html_ret .= '<span style="color:#224279;font-size:36px;font-family:隶书;">' . $info["dharma_name"] . '&nbsp;</span>';
					$html_ret .= '&nbsp;&nbsp;<span style="color:#224279;font-size:24px; margin-left:-30px;font-family:Arial Unicode MS;">' . $info["dharma_pinyin"] . '</span>';
				}
			} else {
				if( $info["dharma_pinyin"]== "") {
					...
					$html_ret .= '<span style="color:#224279;font-size:32px;font-family:隶书;">' . $info["dharma_name"] . '&nbsp;</span>';
					$html_ret .= '&nbsp;&nbsp;<span style="color:#224279;font-size:24px; margin-left:-30px;font-family:Arial Unicode MS;">' . $info["dharma_pinyin"] . '</span>';
				}
			}
	...
	if( $info["group_no"] != "") {
		$html_ret .= '<tr>';
		$html_ret .= '<td valign="middle" align="center">';
		$html_ret .= '<div align="center" style="display:inline-block;width:36px;height:36px;border:1px solid #A01E04;border-radius:18px 18px 18px 18px;">';
		...
		if( $_REQUEST["shoes"] == "1"  && $info["shelf"] != "") 
			$html_ret .= '<span style="margin-left:10px; font-size:16px; font-weight:bold;color:#A01E04;vertical-align:middle;">' . cTYPE::gstr($words["shoes.shelf"]) . ": " . $info["shelf"] . '</span>';
		
		//$html_ret .= '<span style="font-size:30px;color:#A01E04;">' . cTYPE::gstr($words["none.di"]) . ' ' . $info["group_no"] . ' ' .cTYPE::gstr($words["none.zu"]) . '</span>';
		$html_ret .= '</td>';
		$html_ret .= '</tr>';
	}
	...
	$html_ret .= '<div style="position:absolute;top:260px; width:100%; text-align:center; font-family:Arial; font-size:14px;font-weight:bold; color:#A01E04;">';
	$html_ret .= $info["site_name"];
	$html_ret .= '<br>' . $info["phone"];
	$html_ret .= '</div>';
...
function item_photo($i, $j, $info) {
	...
	if( $info["photo"]=="Y") {
		$photo_html = '<img src="' . $CFG["http"] . $CFG["admin_domain"] . '/ajax/lwhUpload_image.php?ts=' . time() . '&size=small&img_id=' .  $info["member_id"] . '" width="100" style="border:1px solid #cccccc; vertical-align:middle;" />';
	} else {
		if( $info["member_id"] > 0 ) 
			$photo_html = '<img src="' . $CFG["http"] . $CFG["admin_domain"] . '/ajax/lwhUpload_image.php?ts=' . time() . '&size=small&img_id=' .  $info["member_id"] . '" width="100" style="border:1px solid #cccccc; vertical-align:middle;" />';
		else 
	...
	$html_ret .= '<div style="position:absolute; top:110px; width:100%; height:160px; border:0px solid red;">';
	$html_ret .= '<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">';
	$html_ret .= '<tr>';
	$html_ret .= '<td colspan="2" valign="middle" align="center">';
	...
	$html_ret .= '<tr>';
	$html_ret .= '<td rowspan="4" align="center" valign="top">';
	$html_ret .= '<div style="display:block;width:100px;margin-left:2px;"><span style="display:inline-block;width:0px;height:100%;vertical-align:middle;"></span>';
	$html_ret .= $photo_html;
	$html_ret .= '</div>';
	$html_ret .= '</td>';
	$html_ret .= '<td>';	$html_ret .= '</td>';
	$html_ret .= '</tr>';
	...
	if( $info["dharma_name"] != "" && $info["dharma_name"]!=$info["name"]) {
		if( $admin_user["lang"] != "en" ) {
			...
			if( cTYPE::iifcn($info["dharma_name"]) )
				$html_ret .= '<span style="font-size:32px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;font-family:隶书;">' . $info["dharma_name"] . '&nbsp;<br>' . $title . '&nbsp;</span>';
			else
				$html_ret .= '<span style="font-size:24px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;font-family:隶书;">' . $info["dharma_name"] . '&nbsp;<br>' . $title . '&nbsp;</span>';
			...
			if( $title != "" ) {
				...
				if( cTYPE::iifcn($title) )
					$html_ret .= '<span style="font-size:24px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;font-family:隶书;">' .$title . '&nbsp;</span>';
				else
					$html_ret .= '<span style="font-size:24px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;font-family:隶书;">' . $title . '&nbsp;</span>';
				...
				if( cTYPE::iifcn($info["dharma_name"]) )
					...
					$html_ret .= '<span style="font-size:24px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;font-family:隶书;">' . $info["dharma_name"] . '&nbsp;</span>';
...
function item_photo1($i, $j, $info) {
	global $CFG;
	global $words;
	global $admin_user;
	$html_ret .= '<div style="position:relative; float:left; border:1px dotted #9E0E94; display:inline-block; margin:1px; text-align:left; width:320px; height:200px; overflow:hidden;">';
	//$html_ret .= '<img src="/theme/blue/image/background/scard.jpg" style="position:absolute; top:0px; left:0px;" width="367" height="320" />';
	$html_ret .= '<div style="position:absolute;top:5px;width:100%;text-align:center;font-family:Arial;font-size:18px;font-weight:bold;color:#A01E04;overflow:hidden;white-space:nowrap;">' . $info["etitle"] . '</div>';
	...
	$html_ret .= '<div style="display:block;width:100%;height:150px;margin-left:2px;"><span style="display:inline-block;width:0px;height:100%;vertical-align:middle;"></span>';
	$html_ret .= $photo_html;
	$html_ret .= '</div>';
	$html_ret .= '</td>';
	...
	if( $info["dharma_name"] != "" && $info["dharma_name"]!=$info["name"]) {
		if( $admin_user["lang"] != "en" ) {
			...
			if( cTYPE::iifcn($info["dharma_name"]) )
				$html_ret .= '<span style="font-size:40px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;font-family:隶书;">' . $info["dharma_name"] . ' ' . $title . '</span>';
			else
			...
			if( $title != "" ) {
				...
				if( cTYPE::iifcn($title) )
					...
					$html_ret .= '<span style="font-size:40px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;font-family:隶书;">' . $title . '</span>';
	...
	$html_ret .= '<div style="position:absolute; top:180px; width:100%; text-align:center; font-family:Arial; font-size:14px; color:#A01E04;">' . $info["edate"] . '</div>';
