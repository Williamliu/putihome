...
function item($i, $j, $info) {
	...
	if( cTYPE::iifcn($info["etitle"]) )
		...
		$html_ret .= '<div style="display:block;width:100%;position:absolute;top:5px;text-align:center;font-family:Arial;font-size:14px;font-weight:bold;color:#A01E04;overflow:hidden;white-space:nowrap;z-index:999;font-family:隶书;">' . $info["etitle"] . '</div>';
	
	$html_ret .= '<div style="position:absolute; top:25px; width:100%; height:140px; border:0px solid red;">';
	...
	if($info["group_no"]!="") {
		...
		if($title != "" && $admin_user["lang"] != "en") {
			$title = '<span style="color:#A01E04;font-size:40px;vertical-align:middle;font-weight:bold;font-family:隶书;">' . $title . '</span>';
		} else {
			$title = '';
		}
		...
		$html_ret .= '<div align="center" style="display:inline-block;vertical-align:middle;width:32px;height:32px;border:1px solid #A01E04;border-radius:16px 16px 16px 16px;">';
	...
	if( $_REQUEST["shoes"] == "1" ) 
		$html_ret .= '<br><center><span style="font-size:16px; font-weight:bold;color:#A01E04;vertical-align:middle;">' . cTYPE::gstr($words["shoes.shelf"]) . ": " . $info["shelf"] . '</span></center>';
