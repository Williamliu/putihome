...
try {
	...
	while( $row = $db->fetch($result)) {
		...
		if( $info["dharma_name"] != "" ) $info["name"] = $info["dharma_name"];
...
function item($i, $j, $info) {
	...
	global $words;
	global $admin_user;
	$html_ret .= '<div style="position:relative; float:left; border:1px dotted #666666; display:inline-block; margin:1px; text-align:left; width:320px; height:190px; overflow:hidden;">';
	//$html_ret .= '<img src="/theme/blue/image/background/scard.jpg" style="position:absolute; top:0px; left:0px;" width="367" height="320" />';
	$html_ret .= '<div style="position:absolute;top:5px;width:100%;text-align:center;font-family:Arial;font-size:28px;font-weight:bold;color:#A01E04;overflow:hidden;white-space:nowrap;">' . $words["other certificate id cover"] . '</div>';

	if( "N"=="Y") {
		$photo_html = '<img src="' . $CFG["http"] . $CFG["admin_domain"] . '/ajax/lwhUpload_image.php?ts=' . time() . '&size=small&img_id=' .  $info["member_id"] . '" height="140" style="border:0px solid #cccccc; vertical-align:middle; margin-top:-20px;" />';
	} else {
	...
	$html_ret .= '<div style="position:absolute; top:48px; width:100%; height:150px; border:0px solid red;">';
	$html_ret .= '<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">';

	$html_ret .= '<tr>';
	$html_ret .= '<td rowspan="3" valign="middle" width="120" align="center">';
	...
	*/
	...
	$html_ret .= '<div style="position:absolute; top:180px; width:100%; text-align:center; font-family:Arial; font-size:14px; color:#A01E04;">&nbsp;</div>';
