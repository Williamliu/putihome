...
function item($i, $j, $info) {
	...
	if( "N"=="Y") {
		$photo_html = '<img src="' . $CFG["http"] . $CFG["admin_domain"] . '/ajax/lwhUpload_image.php?ts=' . time() . '&size=small&img_id=' .  $info["member_id"] . '" height="96" style="border:0px solid #cccccc; vertical-align:middle; margin-top:0px;" />';
	} else {
		$photo_html = '<img src="' . $CFG["http"] . $CFG["admin_domain"] . '/ajax/logo/logo_dharma1.png" height="80" style="border:0px solid #cccccc; vertical-align:middle; margin-top:0px;" />';
	}
	
	$html_ret .= '<div style="position:absolute; top:0px; width:100%; height:100px; border:0px solid red;">';
	...
	if( cTYPE::iifcn($name) )
		$html_ret .= '<span style="font-size:48px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;font-family:隶书;margin-left:-30px;">' . $name . '</span>';
	else
		$html_ret .= '<span style="font-size:32px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;font-family:隶书;margin-left:-30px;">' . $name . '</span>';
