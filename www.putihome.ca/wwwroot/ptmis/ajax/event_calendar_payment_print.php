...
try {
	...
	header("Content-disposition:  attachment; filename=Event_Payment_Report.xls");
	...
	$html.= '<tr>';
	$html.= '<td colspan="14" align="center" height="40" style="font-size:12px; border:0px; font-weight:bold;">' . $row0["title"] . '<br>[ ' . date("M d, Y", $row0["start_date"]) . ($row0["start_date"]>0?' ~ ' .date("M d, Y", $row0["end_date"]):'') .  ' ]</td>';
	$html.= '</tr>';
	...
	$html.= '<td ' . $width_two . ' ' . $header_css . '>' . $words["site"] . '</td>';
	$html.= '<td ' . $width_two . ' ' . $header_css . '>' . $words["paid"] . '</td>';
	$html.= '<td ' . $width_two . ' ' . $header_css . '>' . $words["paid date"] . '</td>';
	$html.= '<td ' . $width_two . ' ' . $header_css . '>' . $words["amount"] . '</td>';
	$html.= '<td ' . $width_two . ' ' . $header_css . '>' . $words["invoice"] . '</td>';
	$html.= '</tr>';
	...
	while($row0 = $db->fetch($result0)) {
		...
		if( $sch_payment != "") {
			 if( intval($sch_payment) == intval($flag_pay) ) {
					...
					$html.= '<td ' . $width_two . ' align="center">' . ($evt_arr["group_no"]>0?$evt_arr["group_no"]:"") . '</td>';
					$html.= '<td ' . $width_two . '>' . $evt_arr["name"] . '</td>';
					$html.= '<td ' . $width_two . '>' . $evt_arr["dharma_name"] . '</td>';
					$html.= '<td ' . $width_two . ' align="center">' . $evt_arr["gender"] . '</td>';
					$html.= '<td ' . $width_two . '>' . $evt_arr["email"] . '</td>';
					$html.= '<td ' . $width_two . '>' . $evt_arr["phone"] . '</td>';
					$html.= '<td ' . $width_two . '>' . $evt_arr["cell"] . '</td>';
					$html.= '<td ' . $width_two . '>' . $evt_arr["city"] . '</td>';
					$html.= '<td ' . $width_two . '>' . $evt_arr["site"] . '</td>';
	...
	$html.= '<tr>';
	$html.= '<td colspan="13" style="font-size:12px; font-weight:bold;">' . $words["total"] . ': ' . $cnt . '</td>';
	$html.= '</tr>';
