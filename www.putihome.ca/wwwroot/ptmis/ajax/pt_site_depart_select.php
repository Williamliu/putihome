...
try {
	...
	$response["data"]["site_id"]	= $_REQUEST["site_id"]; 
...
function depart_items($parent) {
    ...
    $query = "SELECT a.*, IF(ISNULL(b.depart_id), 0, 1) as yes_depart FROM pt_department a LEFT JOIN (SELECT * FROM pt_site_department WHERE site_id = '" . $_REQUEST["site_id"] . "') b ON (a.id = b.depart_id)  WHERE deleted <> 1 AND parent = $parent  ORDER BY sn DESC, created_time ASC";
	...
	while( $row = $db->fetch($result) ) {
        ...
        $item["yes_depart"] = $row["yes_depart"];
