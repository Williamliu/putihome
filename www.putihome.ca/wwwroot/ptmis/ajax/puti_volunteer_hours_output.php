...
try {
	...
	header("Content-disposition:  attachment; filename=volunteer_signature.xls");
	...
	$html.= '<td ' . $width_one . ' ' . $header_css . ' colspan="8" style="height:25px; font-size:14px; font-weight:bold;" align="center">Volunteer Hours Form</td>';
	$html.= '<tr>';
	...
	while( $row = $db->fetch($result)) {
		$cnt++;	
		$html.= '<tr>';
		$html.= '<td ' . $width_one . ' align="center" style="height:25px;">' . $cnt . '</td>';
		$html.= '<td ' . $width_two . '>' . $row["cname"] . '</td>';
		$html.= '<td ' . $width_two . '>' . $row["en_name"] . '</td>';
		$html.= '<td ' . $width_two . '>' . $row["dharma_name"] . '</td>';
		$html.= '<td ' . $width_two . '>' . $row["phone"];
		$html.=  ($row["cell"]!=''?'<br>'.$row["cell"]:'') . '</td>';
	...
	$html.= '<tr>';
	$html.= '<td colspan="8" style="font-size:12px; font-weight:bold;">Total: ' . $cnt . '</td>';
	$html.= '</tr>';
