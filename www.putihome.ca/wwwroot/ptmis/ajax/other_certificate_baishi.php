...
try {
	...
	while( $row = $db->fetch($result)) {
		if( $first ) {
			$first = false;
			if($cnt==0)  $html .= '<center><div style="position:relative; display:block; width:620px; border:0px solid black; margin:0px; padding:0px;">';
		} else {
			if($cnt==0)  $html .= '<center><div style="position:relative; display:block; width:620px; border:0px solid black; margin:0px; padding:0px; page-break-before:always;">';
		}
...
function item_photo($i, $j, $info) {
    ...
    global $puti_site_cn;
    global $puti_site_en;
	$html_ret .= '<div style="position:relative; float:left; border:1px dotted #666666; display:inline-block; margin:1px; text-align:left; width:305px; height:190px; overflow:hidden;">';
	...
	$html_ret .= '<td valign="middle" align="left" style="padding-left:-10px;padding-top:10px;"><span style="font-size:22px;font-weight:bold;color:#A01E04;overflow:hidden;white-space:nowrap;letter-spacing:2px;">' . $words["jinputi"] . '</span></td>';
	$html_ret .= '</tr>';
	...
	$html_ret .= '<td valign="middle" align="center" style="padding-left:-10px;">';
		...
		$html_ret .= '<span style="font-size:12px;color:#A01E04;white-space:nowrap;">' . $words["name"] . '</span>';
		$html_ret .= '</td>';
		$html_ret .= '<td valign="middle" align="left" style="padding-left:10px;">';
		$html_ret .= '<span style="font-size:18px;font-weight:bold;color:#A01E04;overflow:hidden;white-space:nowrap;letter-spacing:' . (cTYPE::iifcn($info["name"])?'5px;':'2px;') .  '">' . $info["name"] . '</span>';
		$html_ret .= '</td>';
		$html_ret .= '</tr>';

		$html_ret .= '<tr>';
		$html_ret .= '<td valign="middle" align="right" width="20" style="white-space:nowrap;">';
		$html_ret .= '<span style="font-size:12px;color:#A01E04;white-space:nowrap;">' . $words["dharma"] . '</span>';
		$html_ret .= '</td>';
		$html_ret .= '<td valign="middle" align="left" style="padding-left:10px;">';
		$html_ret .= '<span style="font-size:32px;font-weight:bold;color:#224279;overflow:hidden;white-space:nowrap;letter-spacing:10px;">' . $info["dharma_name"] . '&nbsp;</span>';
		$html_ret .= '</td>';
		$html_ret .= '</tr>';

		$html_ret .= '<tr>';
		$html_ret .= '<td valign="middle" align="right" width="20" style="white-space:nowrap;">';
		$html_ret .= '<span style="font-size:12px;color:#A01E04;white-space:nowrap;">' . $words["time"] . '</span>';
		$html_ret .= '</td>';
		$html_ret .= '<td valign="middle" align="left" style="padding-left:10px;">';
		$html_ret .= '<span style="font-size:12px;color:#A01E04;overflow:hidden;white-space:nowrap;">' . ( date("Y") . ' ' . $words["yy"] . ' ' . date("n") . ' ' . $words["mm"] . ' ' . date("j") . ' ' . $words["dd"]). '</span>';
		$html_ret .= '</td>';
		$html_ret .= '</tr>';

		$html_ret .= '<tr>';
		$html_ret .= '<td valign="middle" align="right" width="20" style="white-space:nowrap;">';
		$html_ret .= '<span style="font-size:12px;color:#A01E04;white-space:nowrap;">' . $words["loc"] . '</span>';
		$html_ret .= '</td>';
		$html_ret .= '<td valign="middle" align="left" style="padding-left:10px;">';
		
		$html_ret .= '<span style="font-size:12px;color:#A01E04;overflow:hidden;white-space:nowrap;">' . $puti_site_cn . '</span>';
		$html_ret .= '</td>';
		$html_ret .= '</tr>';
