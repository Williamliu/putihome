...
include($CFG["web_path"] . "/source/php_pdf/chinese.php");
...
try {
	...
	*/
	...
	while( $row = $db->fetch($result)) {
		$info["etitle"] 		= $etitle;
		$info["edate"] 			= $edate;
		$info["name"] 			= (strlen($row["first_name"])>9?substr(stripslashes($row["first_name"]),0,9 ) . "." : stripslashes($row["first_name"]) )   . " " . substr(stripslashes($row["last_name"]),0,1) . ".";
		$info["dharma_name"] 	= stripslashes($row["dharma_name"]);	
...
function item($i, $j, $info) {
	...
	$pdf->SetFont('Big5','',14); 
	$pdf->Text($i*$ww + $ww_offset + 5 , $j*$hh + $hh_offset + 30, cTYPE::tobig5($info["etitle"]) );
	...
	$pdf->SetFont('Big5','', 40); 
	$pdf->Text($i*$ww + $ww_offset + 16 , $j*$hh + $hh_offset + 48, cTYPE::tobig5($info["name"]) );

	$pdf->SetFont('Arial','',10); 
	$pdf->Text($i*$ww + $ww_offset + 4 , $j*$hh + $hh_offset + 60, "Dharma: ");
	$pdf->SetFont('Big5','', 18); 
	$pdf->Text($i*$ww + $ww_offset + 20 , $j*$hh + $hh_offset + 60, cTYPE::tobig5($info["dharma_name"]) );
