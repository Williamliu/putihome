...
try {
	...
	$grps = $_REQUEST["member_grp"];
	foreach($grps as $key=>$val) {
		$vvv = explode(":", $val);
		$db->query("UPDATE event_calendar_enroll SET group_no = '" . $vvv[1] . "' WHERE deleted <> 1 AND status = 1 AND id = '" . $vvv[0] . "'");	
	}
	...
	$query1 = "SELECT a.id as enroll_id, a.leader, a.volunteer, b.id, b.first_name, b.last_name, b.dharma_name, b.alias, b.gender, b.email, b.phone, b.cell, b.city, b.site, c.title as site_desc, a.group_no	
						...
						ORDER BY a.group_no ASC, a.leader DESC, a.volunteer DESC, a.created_time ASC";
	...
	while( $row1 = $db->fetch($result1) ) {
		...
		$grp_arr["site_desc"]	= $row1["site_desc"];
