...
<head>
		...
		$(function(){
			  ...
			  ctt = new LWH.cTABLE({
											...
											headers:[
												...
												{title: words["dharma pinyin"], col:"dharma_pinyin",sq:"ASC"},
												{title: words["apply.date"], 	col:"apply_date",	sq:"ASC"},
												{title:"", 						col:""},
												...
												{title: words["short.lang"], 	col:"language", 	align:"center",	sq:"ASC"},
											...
											url:			"ajax/dharma_name_select.php",
			...
			$(".tabQuery-button[oper='print']").live("click", function(ev){
					  ...
					  $.ajax({
						  ...
						  type: "post",
						  url: "ajax/dharma_name_card_print.php"
					  });
			});	
			  ...
			  $(".tabQuery-button-save").live("click", function(ev) {
					  var mid = $(this).attr("rid");
					  var adate = $(".apply_date[rid='" + mid + "']").val();
					  var tname = $(".tmp_dharma_name[rid='" + mid + "']").val();
					  var pinyin = $(".tmp_dharma_pinyin[rid='" + mid + "']").val();
					  var dlist = [];
					  dlist[0] = {};
					  dlist[0].member_id 			= mid;
					  dlist[0].apply_date 			= adate;
					  dlist[0].temp_dharma_name 	= tname;
					  dlist[0].temp_dharma_pinyin 	= pinyin;
					  $.ajax({
						  data: {
							  admin_sess: 	$("input#adminSession").val(),
							  admin_menu:	$("input#adminMenu").val(),
						  ...
						  error: function(xhr, tStatus, errorTh ) {
							  $("#wait").loadHide();
							  alert("Error (dharma_name_save.php): " + xhr.responseText + "\nStatus: " + tStatus);
						  },
						  success: function(req, tStatus) {
							  $("#wait").loadHide();
							  if( req.errorCode > 0 ) { 
						  ...
						  type: "post",
						  url: "ajax/dharma_name_save.php"
					  });
			  });
			  
			  
			  $(".btn_save_all").live("click", function(ev) {
					  var dlist = [];
					  $(".tmp_dharma_name", "#event_enrollment").each(function(idx, el) {
							var mid = $(el).attr("rid");
							var adate = $(".apply_date[rid='" + mid + "']").val();
							var pinyin = $(".tmp_dharma_pinyin[rid='" + mid + "']").val();
							var tname = $(el).val();
							dlist[idx] = {};
							dlist[idx].member_id 			= mid;
							dlist[idx].apply_date 			= adate;
							dlist[idx].temp_dharma_name 	= tname;
							dlist[idx].temp_dharma_pinyin 	= pinyin;
					  });
			  ...
			  $("#btn_close_all").live("click", function(ev) {
				  var yes = false;
				  yes = window.confirm( words["are you sure to publish dharma name?"] );
				  if(!yes) return;
					  $.ajax({
						  data: {
							  admin_sess: 	$("input#adminSession").val(),
						  ...
						  error: function(xhr, tStatus, errorTh ) {
							  $("#wait").loadHide();
							  alert("Error (dharma_name_close.php): " + xhr.responseText + "\nStatus: " + tStatus);
						  },
						  success: function(req, tStatus) {
							  $("#wait").loadHide();
							  if( req.errorCode > 0 ) { 
						  ...
						  type: "post",
						  url: "ajax/dharma_name_close.php"
					  });
			  });
		...
		function print_certificate() {
					  ...
					  $.ajax({
						  ...
						  success: function(req, tStatus) {
							  $("#wait").loadHide();
							  var w1 = window.open("output.html");
							  var html_str = "<" + "html" + "><" + "head" + "><" + "title" + ">Puti Meditation Dharma Name Card</" + "title" + "></" + "head" + "><" + "body>";
							  w1.document.open();
							  w1.document.write(html_str);
							  w1.document.write(req.data);
							  w1.document.write('</html>');
		...
		function pageDONE() {
			  ...
			  var btn_save = '<tr><td colspan="13" align="center"><input type="button" class="btn_save_all" right="save" value="' + words["save all"] + '" /></td></tr>';
			  $("#event_enrollment table").append(btn_save);
		}
		...
		function pageHTML( pRows ) {
			...
			for(var idx in pObjs) {
				...
				html += '<td><span>';
				html += '<input class="tmp_dharma_name" rid="'  + pObjs[idx]["id"] + '" style="width:60px; color:#CB393A;font-weight:bold;font-size:16px;" value="' + pObjs[idx]["temp_dharma_name"] + '" />'; //pObjs[idx]["dharma_name"];
				html += '</span></td>';

				html += '<td><span>';
				html += '<input class="tmp_dharma_pinyin" rid="'  + pObjs[idx]["id"] + '" style="width:100px; color:#CB393A;font-weight:bold;font-size:16px;" value="' + pObjs[idx]["temp_dharma_pinyin"] + '" />'; //pObjs[idx]["dharma_name"];
				html += '</span></td>';

				html += '<td style="white-space:nowrap;"><span>';
				html += '<input class="apply_date"  rid="'  + pObjs[idx]["id"] + '" style="width:80px; font-size:12px;" value="' + pObjs[idx]["apply_date"] + '" />'; //pObjs[idx]["dharma_name"];
				html += '</span></td>';

				html += '<td><span>';
				html += '<a class="tabQuery-button tabQuery-button-save" oper="save" right="save" 	rid="' +  pObjs[idx]["id"] + '" title="' +  words["button save"] + '"></a>';					 
				html += '</span></td>';
		...
		function print_event() {
				  if( $("iframe[name='ifm_list_excel']").length > 0 ) {
						...
						$("form[name='frm_list_excel']").attr({"action":"ajax/dharma_name_output.php", "target": "ifm_list_excel" }); 
              ...
              <table cellpadding="2" cellspacing="0">
                  ...
                  <tr>
                      ...
                      <td>
                         ...
                         <span style="margin-left:20px;font-size:12px;"><?php echo $words["apply.date"]?>: </span>											
                  ...
                  <tr>
                      ...
                      <td colspan="5">
                        ...
                        <input type="button" class="btn_save_all" right="save" value="<?php echo $words["save all"]?>" />
                        <input type="button" oper="search" right="print" onclick="print_certificate()" value="<?php echo $words["print dharma name"]?>" />                  
                        <input type="button" id="btn_close_all" right="save" style="margin-left:30px;" value="<?php echo $words["button close all"]?>" />
                    </td>
                  </tr>
              </table>
    </fieldset>
