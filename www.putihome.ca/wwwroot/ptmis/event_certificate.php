...
<head>
		...
		$(function(){
			  ...
			  ctt = new LWH.cTABLE({
											...
											url:			"ajax/event_certificate_select.php",
			...
			$(".tabQuery-button-save").live("click", function(ev) {
				...
				dobj.cert_no	= $(".certificate[rid='" + dobj.member_id + "']").val();
				dobjs[0] = dobj;
				save_ajax(dobjs);				
			});
			...
			if( $("#cert_temp").val() != "" ) {
					  ...
					  $.ajax({
						  ...
						  success: function(req, tStatus) {
							  $("#wait").loadHide();
							  var w1 = window.open("output.html");
							  var html_str = "<" + "html" + "><" + "head" + "><" + "title" + ">Bodhi Meditation Certification</" + "title" + "></" + "head" + "><" + "body>";
							  w1.document.open();
							  w1.document.write(html_str);
							  w1.document.write(req.data);
							  w1.document.write('</html>');
		...
		function pageHTML( pRows ) {
			...
			for(var idx in pObjs) {
				...
				html += '<td>';
				html += '<input class="certificate" type="text" style="width:150px;font-size:10px;" rid="'  + pObjs[idx]["id"] + '" value="' + pObjs[idx]["cert_no"] + '" />';
				html += '</td>';

				html += '<td align="center"  style="white-space:nowrap;">';
			 	//html += '<a class="enroll_button_save" 		oper="save" 		right="save" 	rsn="' + idx + '"	rid="' + pObjs[idx]["id"] + '" title="' + words["enroll"] + '"></a>';
				html += ' <a class="tabQuery-button tabQuery-button-save" 		oper="save" 	right="save" 	rsn="' + idx + '"	rid="' + pObjs[idx]["id"] + '" title="' + words["button save"] + '"></a>';
		...
		function print_event() {
				  if( $("iframe[name='ifm_list_excel']").length > 0 ) {
						...
						$("form[name='frm_list_excel']").attr({"action":"ajax/event_certificate_print.php", "target": "ifm_list_excel" }); 
		...
		function save_all() {
			...
			$(".certificate").each(function(idx, el) {
				var dobj = {};
		...
		function save_ajax(docObj) {
			  ...
			  $.ajax({
				  ...
				  error: function(xhr, tStatus, errorTh ) {
					  $("#wait").loadHide();
					  alert("Error (event_certificate_save.php): " + xhr.responseText + "\nStatus: " + tStatus);
				  },
				  success: function(req, tStatus) {
					  $("#wait").loadHide();
					  if( req.errorCode > 0 ) { 
				  ...
				  type: "post",
				  url: "ajax/event_certificate_save.php"
			  });
		}
		...
		function clear_ajax() {
			  ...
			  $.ajax({
				  ...
				  error: function(xhr, tStatus, errorTh ) {
					  $("#wait").loadHide();
					  alert("Error (event_certificate_clear.php): " + xhr.responseText + "\nStatus: " + tStatus);
				  },
				  success: function(req, tStatus) {
					  $("#wait").loadHide();
					  if( req.errorCode > 0 ) { 
				  ...
				  type: "post",
				  url: "ajax/event_certificate_clear.php"
			  });
		}
        </script>
...
?>
                      ...
                      <td colspan="5">
					    ...
					    <span style="margin-left:10px;vertical-align:middle;font-size:14px; font-weight:bold;"><?php echo $words["cert.temp"]?>: 
                        	<select id="cert_temp" name="cert_temp">
                            	<option value=""></option>
                                <option value="event_certificate_level1.php"><?php echo $words["cert.level1"]?></option>
                                <option value="event_certificate_level11.php"><?php echo $words["cert.level11"]?></option>
                                <option value="event_certificate_level12.php"><?php echo $words["cert.level12"]?></option>  
                                <option value="event_certificate_bagua1.php"><?php echo $words["cert.bagua1"]?></option>
                                <option value="event_certificate_level2.php"><?php echo $words["cert.level2"]?></option>
                                <option value="event_certificate_level3.php"><?php echo $words["cert.level3"]?></option>
                                <option value="event_certificate_level31.php"><?php echo $words["cert.level31"]?></option>
                            </select>
						</span>  
                        ...
                        <input type="button" right="save" style="width:60px;margin-left:20px;" onclick="clear_ajax()" value="<?php echo $words["button clear"]?>" />                  
                    </td>
                  </tr>
              </table>
    </fieldset>
	...
	<center>
		<input type="button" oper="save" right="save" style="width:100px;" onclick="save_all()" value="<?php echo $words["button save all"]?>" />                           
    </center>
<?php 
