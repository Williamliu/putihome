...
<head>
		...
		$(function(){
			  ...
			  $(".event-item").die("click").live("click", function(ev) {
			  		var sid = $(this).attr("siteid");
					var bid = $(this).attr("branchid");
					
					list_event(sid, bid);
			  });
		});
		
		function list_event(sid, bid) {
			$.ajax({
				data: {
					lang:	'<?php echo $Glang;?>',
					site: 	sid,  //'<?php echo $_REQUEST["site"];?>',
					branch: bid   //'<?php echo $_REQUEST["branch"];?>'
				},
				dataType: "json",  
				...
				success: function(req, tStatus) {
					if( req.errorCode > 0 ) { 
						...
						$(".event-item[siteid='" + req.data.site + "'][branchid='" + req.data.branch + "']").addClass("selected");
		...
		function jsonToHTML( sitesObj ) {
			...
			for(var key0 in sitesObj) {
				...
				html += '<span class="click" style="color:#840F71; font-size:16px; font-weight:bold;">' + words[osite.title.toLowerCase()] + '</span>'; 
